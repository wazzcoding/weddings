...
function setGuestName(name){
  const n=name.trim().slice(0,30) || "Nama Tamu";
  if(guestNameEl) guestNameEl.textContent=n;
  if(wNameInput) wNameInput.value=n;
}
(function initGuest(){
  const params=new URLSearchParams(window.location.search);
  const namaTamu=(params.get("to")||"").trim().slice(0,30);
  if(namaTamu) setGuestName(namaTamu);
  else if(guestNameEl) guestNameEl.textContent="Nama Tamu";
})();
